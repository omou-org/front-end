describe('Editing an existing Receptionist account details', () => {
    before(() => {
        cy.fixture('users.json').then(
            ({ original_receptionist_data, updated_receptionist_data }) => {
                cy.mockGraphQL({
                    CreateAdmin: {
                        response: {
                            data: {
                            ...
                            Object.entries(variables).forEach(
                                ([key, value]) => {
                                    if (
                                        ...
                                        expect(
                                            updated_receptionist_data[key] ||
                                                updated_receptionist_data?.user[
                                                    key
                                                ]
                                        ).equals(value);
                                    }
                    ...
                    GetAdmin: {
                        ...
                        test: ({ userID }) => {
                            expect(userID).equals(
                                original_receptionist_data.user.id.toString(),
                                'Check ID passed'
                            );
                        },
                    },
                ...
                cy.visitAuthenticated(
                    `/form/admin/${original_receptionist_data.user.id}`
                );
            }
        );
    });
    ...
    it('Loads existing receptionist account login details form section for receptionist', () => {
        cy.fixture('users.json').then(({ original_receptionist_data }) => {
            cy.get('[data-cy=login-firstName-input]').should(
                'have.value',
                original_receptionist_data.user.firstName
            );
            cy.get('[data-cy=login-lastName-input]').should(
                'have.value',
                original_receptionist_data.user.lastName
            );
            cy.get('[data-cy=login-email-input]').should(
                'have.value',
                original_receptionist_data.user.email
            );
            cy.get('[data-cy=login-nextButton]').should('be.disabled');
        });
    });
    ...
    it('Edit receptionist data in Login Details section of the form', () => {
        cy.fixture('users.json').then(({ updated_receptionist_data }) => {
            ...
            cy.get('[data-cy=login-email-input]').fastType(
                updated_receptionist_data.user.email
            );
            cy.get('[data-cy=login-nextButton]').should('be.disabled');
    ...
    it('Loads receptionist data into User Information section of receptionist editing form', () => {
        cy.fixture('users.json').then(({ original_receptionist_data }) => {
            cy.get('[data-cy=login-nextButton').should('be.enabled').click();
            cy.get('[data-cy=user-adminType-select]')
                .siblings()
                .should('have.value', original_receptionist_data.adminType);
            cy.get('[data-cy=user-address-input]').should(
                'have.value',
                original_receptionist_data.address
            );
            cy.get('[data-cy=user-phoneNumber-input]')
                .should(
                    'have.value',
                    original_receptionist_data.phoneNumber.replace(
                        /(\d{3})(\d{3})(\d{4})/,
                        '($1) $2-$3'
                    )
                )
