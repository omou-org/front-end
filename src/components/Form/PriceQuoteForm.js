      ...
      instance.post("/pricing/quote/", requestedQuote).then((quoteResponse) => {
        ...
        if (responseDiscounts !== stateDiscounts) {
          let ResponseDiscounts = quoteResponse.data.discounts.map(
              (discount) => ({
                ...
                enable: discounts.find(
                    ...
                    ? discounts.find((sDiscount) => sDiscount.id === discount.id)
                        .enable
                    : true,
              })
          );
          ...
          const discountNotInResponseButInState = discounts.filter(
              (discount) => !ResponseDiscountIDs.includes(discount.id)
          );
  ...
  const handlePay = () => (e) => {
    ...
    tutoring.forEach((tutoring) => {
      ...
      const tutoringCourse = {
        ...
        academic_level:
            tutoring.academic_level || tutoring.new_course.academic_level,
        instructor:
            tutoring.new_course.instructor || tutoring.new_course.instructor_id,
              ...
              <Grid container direction="column" spacing={1}>
                ...
                <Grid item>
                  ...
                  <Grid container direction="row" justify="flex-end">
                    ...
                    <Grid item xs={2}>
                      <Typography align="right" className="total-price">
                        ${priceQuote.total}
                      </Typography>
                    </Grid>
                  </Grid>
