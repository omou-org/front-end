...
const PriceQuoteForm = ({ courses, tutoring }) => {
    ...
    const handlePayMethodChange = useCallback(
        (method) => () => {
    ...
    useEffect(() => {
        if (
            ...
            const requestedQuote = {
                ...
                disabled_discounts: discounts
                    .filter((discount) => !discount.enable)
                    .map(({ id }) => id),
            ...
            instance
                .post('/pricing/quote/', requestedQuote)
                .then((quoteResponse) => {
                    const responseDiscounts = JSON.stringify(
                        quoteResponse.data.discounts
                    );
                    ...
                    if (
                        quoteResponse.data.price_adjustment !== priceAdjustment
                    ) {
    ...
    const handlePay = () => (e) => {
        ...
        tutoring.forEach((tutoring) => {
            const startDate = tutoring.new_course.schedule.start_date.substring(
                0,
                10
            );
            const endDate = tutoring.new_course.schedule.end_date.substring(
                0,
                10
            );
            const tutoringCourse = {
                ...
                day_of_week:
                    dayOfWeek[tutoring.new_course.day_of_week] ||
                    tutoring.new_course.schedule.days,
                ...
                max_capacity: 1,
                course_category:
                    tutoring.category_id || tutoring.new_course.category,
                academic_level:
                    tutoring.academic_level ||
                    tutoring.new_course.academic_level,
                instructor:
                    tutoring.new_course.instructor ||
                    tutoring.new_course.instructor_id,
            ...
            tutoringRegistrations.push({
                ...
                sessions: isExistingTutoring(tutoring.courseID)
                    ? tutoring.sessions
                    : weeklySessionsParser(startDate, endDate),
        ...
        (id) => () => {
            setDiscounts((previousDiscounts) =>
                previousDiscounts.map((discount) => ({
                    ...discount,
                    enable:
                        discount.id === id ? !discount.enable : discount.enable,
                }))
            );
        },
        []
