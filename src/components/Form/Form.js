	...
	componentWillMount() {
				...
				case "course": {
					if (id && this.props.registeredCourses) {
						if (id.indexOf("+") >= 0) {
							...
							const {form} = this.props.registeredCourses[studentID].find(
		...
		if (
			...
			if (formType === "tutoring" && this.props.courses.hasOwnProperty(id)) {
				prevState["Tutor Selection"]["Course / Subject"] = this.props.courses[
					id
					].title;
			}
				...
				case "student": {
					...
					(async () => {
						...
						try {
							...
							const parents = await apiActions.instance.get("/account/parent/");
							this.props.dispatch({
			...
			(oldState) => {
				if (this.validateSection()) {
						...
						if (!oldState.submitPending) {
							if (this.props.match.params.edit === "edit") {
								...
								switch (this.state.form) {
									case "small_group":
										if (
											this.state["Group Type"]["Select Group Type"] ===
											"New Small Group"
										) {
									...
									case "pricing":
										...
										break;
									case "discount":
										const discountType = this.state["Discount Description"][
											"Discount Type"
											];
					...
					if (conditionalField) {
						...
						// create blank fields based on form type
						newState[title] = {};
						formContents[nextActiveSection][conditionalField].forEach(
							(field) => {
					...
					return (
						<div>
							<Typography>
								...
								<b>${course.hourly_tuition}</b>. You will select the number of
			...
			case "select":
				...
				return (
					<FormControl className="form-control">
						<InputLabel shrink={Boolean(value)}>{fieldTitle}</InputLabel>
						<Select
							disabled={disabled}
						...
						>
							{options.map((option) => (
								<MenuItem
									key={option}
									selected={
										option === "1 Hour" && fieldTitle === "Duration"
											? true
											: fieldTitle !== "Duration"
									}
									value={option}
			...
			case "course": {
				...
				return (
					<div style={{width: "inherit"}}>
						<Grid className="student-align" container spacing={2000}>
							<SearchSelect
			...
			case "course_details":
				if (
					...
					// add duration and weeks for the end date
						...
						end.getDate() + (this.state.Tuition["# of Weekly Sessions"] - 1) * 7
					);
				}
				break;
			case "tutoring":
					...
					<Step key={label}>
						...
						<StepContent>
							{section.map((field, j) => {
								...
								const numSameTypeFields = section.reduce(
									(count, otherField) =>
										field.field === otherField.field ? count + 1 : count,
									0
									),
									reversedSection = [...section].reverse(),
									lastFieldOfType = reversedSection.find(
										(otherField) => otherField.field === field.field
									);
								return (
										...
										>
											{activeStep === steps.length - 1
												? this.props.submitPending
													? "Submitting"
													: ["course", "tutoring"].includes(this.state.form)
														? "Add to Cart"
														: "Submit"
												: "Next"}
										</Button>
										...
										<div key={field}>
											...
											<Typography align="left" className="field-value">
												{typeof fieldVal === "object"
													? new Date(fieldVal).toLocaleString("eng-US")
													: fieldVal || "N/A"}
											</Typography>
										</div>
									);
					...
					<Paper className="registration-form paper">
						...
						{this.props.submitStatus !== "success" ? (
							this.props.registrationForm[this.state.form] ? (
								...
								<Typography>Sorry! The form is unavailable.</Typography>
							)
						...
						>
							...
							<DialogContent>
								<DialogContentText id="alert-dialog-description">
									There was an error submitting the form. Check all fields and
									try again.
								</DialogContentText>
							</DialogContent>
							<DialogActions>
