...
const TutoringPriceQuote = ({
	...
	const filteredPriceRules = priceRules.filter(rule => rule.courseType === courseType);
	const categoryOptions = filteredPriceRules.map(({category}) => ({
		label: category.name,
		value: category.id,
	}))
		.filter((rule, index, self) => index === self.findIndex((r) => (r.value === rule.value)));
	const academicLevelOptions = filteredPriceRules.map(({academicLevel, category, id}) => ({
		label: {
			"COLLEGE_LVL": "College",
		...
		.filter((rule) => (
			rule.value.category === categoryValue
		));
					...
					>
						{
							academicLevelOptions.map((academicLevel, i) => (
								<MenuItem key={i} value={academicLevel.value.ruleId}>
									{academicLevel.label}
								</MenuItem>
							))
						}
