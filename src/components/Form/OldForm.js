...
class Form extends Component {
                        ...
                        prevState = {
                            ...
                            Experience: {
                                'Subject(s) Tutor Can Teach':
                                    instructor.background.subjects,
                ...
                case 'course': {
                    if (id && this.props.registeredCourses) {
                        if (id.indexOf('+') >= 0) {
                            ...
                            const { form } = this.props.registeredCourses[
                                studentID
                ...
                this.setState(
                    (oldState) => {
                        const formContents = JSON.parse(
                            JSON.stringify(
                                this.props.registrationForm[formType]
                            )
                        );
                        const NewState = {
                            ...oldState,
    ...
    componentDidMount() {
        ...
        if (
            !this.props.isAdmin &&
            (formType === 'instructor' || formType === 'course_details')
        ) {
                ...
                case 'student': {
                    ...
                    (async () => {
                        ...
                        try {
                            ...
                            if (student) {
                                const parent = this.props.parents[
                                    student.parent_id
                                ];
                                this.setState((prevState) => ({
                                    'Basic Information': {
                ...
                case 'course_details': {
                    ...
                    (async () => {
                        ...
                        try {
                            ...
                            if (course) {
                                const inst = this.props.instructors[
                                    course.instructor_id
                                ];
                                this.setState(
                                    utils.loadEditCourseState(course, inst)
                                );
                            }
                        }
                    })();
    ...
    validateSection() {
        ...
        if (this.state.isSmallGroup) {
            ...
            currSectionTitle = this.getFormObject().section_titles[
        ...
        return (
            ...
            Object.values(this.state[`${currSectionTitle}_validated`]).every(
                (valid) => valid
            )
        );
    }
    ...
    handleNext = () => {
        let currSectionTitle = this.getFormObject().section_titles[
            this.state.activeStep
        ];
        let section = this.props.registrationForm[this.state.form][
        ...
        section.forEach((field) => {
            this.validateField(
                currSectionTitle,
                field,
                this.state[currSectionTitle][field.name]
            );
        });
        this.setState(
            (oldState) => {
                if (this.validateSection()) {
                    if (
                        oldState.activeStep ===
                            this.getFormObject().section_titles.length - 1 ||
                        oldState.isSmallGroup
                    ) {
                        if (!oldState.submitPending) {
                            if (this.props.match.params.edit === 'edit') {
                                this.props.registrationActions.submitForm(
                                    this.state,
                                    this.props.match.params.id
                                );
                            } else {
                                switch (this.state.form) {
                                    case 'small_group':
                                        if (
                                            this.state['Group Type'][
                                                'Select Group Type'
                                            ] === 'New Small Group'
                                        ) {
                                            this.props.apiActions.submitNewSmallGroup(
                                                this.state
                                            );
                                        } else {
                                            this.props.registrationActions.submitForm(
                                        ...
                                        break;
                                    case 'pricing':
                                        this.props.adminActions.setPrice(
                                            this.state
                                        );
                                        break;
                                    case 'discount':
                                        ...
                                        const discountPayload = utils.createDiscountPayload(
                                            this.state,
                                            discountType
                                        );
                                        this.props.adminActions.setDiscount(
                                            discountType,
                                            discountPayload
                                        );
                                        break;
                    ...
                    const conditionalField = this.getConditionalFieldFromCurrentSection(),
                        ...
                        nextActiveSection = this.getFormObject().section_titles[
                            nextActiveStep
                        ];
                    const newState = {
                    ...
                    if (conditionalField) {
                        ...
                        // create blank fields based on form type
                        newState[title] = {};
                        formContents[nextActiveSection][
                            conditionalField
                        ].forEach((field) => {
            ...
            (oldState) => {
                if (oldState.activeStep !== 0 && oldState.activeSection) {
                    return {
                        ...
                        activeSection: this.getFormObject().section_titles[
                            oldState.activeStep - 1
                        ],
                    };
                }
                return {};
    ...
    validateField(sectionTitle, field, fieldValue) {
        this.setState(
            (oldState) => {
                if (!fieldValue) {
                    // if empty field
                    oldState[`${sectionTitle}_validated`][
                        field.name
                    ] = !field.required;
                    ...
                    if (field.type === 'number') {
                        // parse if number
                        oldState[sectionTitle][field.name] = parseInt(
                            fieldValue,
                            10
                        );
                        ...
                        isValid =
                            !emails.includes(fieldValue) || oldState.preLoaded;
                        if (!isValid) {
        ...
        switch (field.type) {
            case 'price quote':
                if (
                    ...
                    this.state.conditional.toLowerCase().includes('existing')
                ) {
                    const course = this.props.courses[
                        this.state['Group Details']['Select Group'].value
                    ];
                    return (
                        <div>
                            <Typography>
                                <b>{course.title}'s</b> hourly rate is{' '}
                                <b>${course.hourly_tuition}</b>. You will select
                                the number of sessions you wish to register for
                                at checkout.
                            </Typography>
                        </div>
                    );
                    ...
                    return (
                        <TutoringPriceQuote
                            ...
                            handleUpdatePriceFields={this.updatePriceFields.bind(
                                this
                            )}
            ...
            case 'course': {
                        ...
                        .filter(
                            (courseID) =>
                                this.props.courses[courseID].capacity >
                                    this.props.courses[courseID].roster
                                        .length &&
                ...
                courseList = this.removeDuplicates(
                    Object.values(this.state[label]),
                    courseList
                );
                // count # of course fields in current section
                const fieldCount = this.getActiveSection().reduce(
                    (total, { type }) => total + (type === 'course'),
                    0
                );
                return (
            ...
            case 'student': {
                let studentList = [];
                if (this.props.currentParent) {
                    this.props.currentParent.student_list.forEach(
                        (studentID) => {
                    ...
                    studentList = Object.values(this.props.students).map(
                        ({ user_id, name, email }) => ({
                            value: user_id,
                            label: `${name} - ${email}`,
                        })
                ...
                studentList = this.removeDuplicates(
                    Object.values(this.state[label]),
                    studentList
                );

                // count # of course fields in current section
                const studentCount = this.getActiveSection().reduce(
                    (total, { type }) => total + (type === 'student'),
                    0
                );
            ...
            case 'category': {
                const categoriesList = this.props.courseCategories.map(
                    ({ id, name }) => ({
                        value: id,
                        label: name,
                    })
                ...
                if (currVal && typeof currVal !== 'object') {
                    const category = this.props.courseCategories.find(
                        ({ id }) => Number(id) === Number(currVal)
                    );
                    if (category) {
                        this.setState((oldState) => ({
                            ...oldState,
            ...
            case 'time':
                ...
                if (
                    this.state[label][fieldTitle] &&
                    typeof this.state[label][fieldTitle] !== 'string'
                ) {
            ...
            default:
                const textValue =
                    utils.weeklySessionsParser(this.state[label], field.name) ||
                    this.state[label][field.name];
                return (
                    <TextField
                        disabled={disabled}
    ...
    addField(field, fieldIndex) {
        ...
        this.setState(
            (prevState) => {
                ...
                const numSameTypeFields = section.reduce(
                    (count, otherField) => count + (field === otherField.field),
                    0
                );
                if (Array.isArray(prevState[activeSection])) {
                    prevState[activeSection][
                        `${field} ${numSameTypeFields}`
                    ] = null;
                    prevState[`${activeSection}_validated`][
                        `${field} ${numSameTypeFields}`
                    ] = true;
                } else {
                    prevState[activeSection][
    ...
    removeField(fieldIndex) {
        this.setState(
            (prevState) => {
                ...
                const currentSectionValidationFields =
                    prevState[`${prevState.activeSection}_validated`];
                let baseName;
                ...
                Object.entries(currentSectionFields).forEach(
                    ([origFieldKey, fieldValue]) => {
                        ...
                        newSectionValidationFields[newFieldName] =
                            currentSectionValidationFields[origFieldKey];
                        index++;
                    }
                );
            ...
            () => {
                ...
                this.props.registrationActions.removeField(
                    param,
                    fieldIndex,
                    this.state.conditional
                );
    ...
    renderForm() {
        ...
        let currentForm, steps;
        if (this.state.isSmallGroup) {
            const { form_type, Student } = this.props.registrationForm[
                this.state.form
            ];
            currentForm = {
                form_type,
        ...
        switch (this.state.form) {
            case 'course_details':
                if (
                    this.state.activeSection === 'Tuition' &&
                    this.state.Tuition &&
                    this.state.Tuition.Duration
                ) {
                    instructorID = this.state['Course Info']['Instructor']
                        .value;
                    ...
                    // add duration and weeks for the end date
                            ...
                            durationStringToNum[this.state.Tuition.Duration] *
                                60
                    );
                    end.setDate(
                        end.getDate() +
            ...
            case 'tutoring':
                if (
                    this.state.activeSection === 'Tuition Quote Tool' &&
                    this.state.Schedule &&
                    this.state.Schedule['Number of Sessions'] &&
                    this.state.Schedule.Duration
                ) {
                    instructorID = this.state['Tutor Selection']['Instructor']
                        .value;
                    ...
                    // add duration and weeks for the end date
                        ...
                        end.getMinutes() + durationStringToNum[Duration] * 60
                    );
            ...
            case 'small_group':
                if (
                    ...
                    instructorID = this.state['Group Details']['Instructor']
                        .value;
                    const numSesh = this.state['Group Details'][
                        '# of Weekly Sessions'
                    ];
                        ...
                        <StepContent>
                            {section.map((field, j) => {
                                ...
                                const numSameTypeFields = section.reduce(
                                    ...
                                    lastFieldOfType = reversedSection.find(
                                        (otherField) =>
                                            otherField.field === field.field
                                    );
                                return (
                                    <div
                        ...
                        return (
                            <div key={sectionTitle}>
                                ...
                                {sectionFields.map(({ field, type }) => {
                                    let fieldVal = this.state[sectionTitle][
                                        field
                                    ];
                                    if (
                                        fieldVal &&
                                        fieldVal.hasOwnProperty('value')
                                    ) {
    ...
    renderCourseRegistrationSubmission() {
        if (this.props.registeredCourses) {
            ...
            let registeredCourseForm = this.props.registeredCourses[
                currentStudentID
            ];
            registeredCourseForm =
                registeredCourseForm[registeredCourseForm.length - 1];
            return (
                <>
    ...
    renderTitle(id, type) {
        ...
        return `${title} ${type.split('_').join(' ')} ${
            this.props.match.params.edit === 'edit' ? 'Edit' : 'Registration'
        }`;
    }
