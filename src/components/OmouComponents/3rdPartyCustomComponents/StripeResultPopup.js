...
const StripeResultPopup = ({ isConnectedToStripe }) => {
    ...
    const [integrationSucceeded, setIntegrationSucceeded] = useState(
        didStripeOnboardingFlowTimeout
    );

    useEffect(() => {
        if (integrationSucceeded !== null) {
            setIntegrationSucceeded(
                didStripeOnboardingFlowTimeout === 'false' &&
                    isConnectedToStripe
            );
    ...
    const successfulPopup = (
        <Dialog open={displayPopup} onClose={handleClose}>
            <DialogTitle disableTypography id='dialog-title'>
                Congratulations!
            </DialogTitle>
            <DialogContent>
                <DialogContentText id='dialog-description'>
                    Your business is now accepting credit card payments through
                    Omou powered by Stripe. Please contact support if there are
                    any issues.
                </DialogContentText>
            </DialogContent>
            <DialogActions>
        ...
        <Dialog open={displayPopup} onClose={handleClose}>
            ...
            <DialogContent>
                <DialogContentText id='dialog-description'>
                    ...
                    Please try again without refreshing or taking too long to
