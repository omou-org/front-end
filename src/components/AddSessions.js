...
const AddSessions = ({
	...
	const handleRegisterMoreSessions = (event) => {
		...
		if (
			registeringParent &&
			registeringParent !== "none" &&
			registeringParent.user.id !== parentOfCurrentStudent
		) {
			...
			goToRoute("/registration/cart/");
		} else if (!registeringParent || registeringParent === "none") {
			api.setParentAddCourseRegistration(
				parentOfCurrentStudent,
				courseToRegister(
					enrollment,
					courses[enrollment.course_id],
	...
	const renderComponent = () => {
		switch (componentOption) {
			...
			case "menuItem": {
				return (
					<MenuItem onClick={handleRegisterMoreSessions}>Add Sessions</MenuItem>
				);
			}
			default:
				return null;
				...
				<DialogContent>
					<DialogContentText>
                        ...
                        You are currently registering ${
							registeringParent &&
							registeringParent !== "none" &&
							registeringParent.user.name
						}. If you wish to continue to add sessions, you will
                        discard all of the currently registered courses with this parent.
                        `}
					</DialogContentText>
				</DialogContent>
				<DialogActions>
					<Button color="secondary" onClick={closeDiscardParentWarning(true)}>
						Continue & Add Session
					</Button>
					<Button color="primary" onClick={closeDiscardParentWarning(false)}>
						Cancel
					</Button>
				</DialogActions>
			</Dialog>
