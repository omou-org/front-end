...
const UnpaidSessionCard = ({unpaidStudent}) => {
	...
	return (
		<Card className="unpaid-sessions-card">
			<CardMedia>
				<Grid className="unpaid-avatar-container" container>
					<Avatar
			...
			<CardContent className="unpaid-details">
				<Typography className="unpaid-student-name">{student.name}</Typography>
				<Typography className="unpaid-role-label">Student</Typography>
				<Typography>
					Payment Status:{" "}
					<span
						...
						style={{
							backgroundColor:
								adminUtils.statusColor[unpaidStudent.sessions_left],
						}}
					>
            {unpaidStudent.sessions_left}
          </span>
