...
const ActionLog = () => {
    ...
    const handleUserSelection = (event) => {
        ...
        setCurrentId(
            data.admins.filter((user) => user.user.id === event.target.value)[0]
                .user.id
        );
    };
    ...
    const renderName = (actionId) => {
        const selectedUser = data.admins.find(
            (user) => user.user.id == actionId
        );
                ...
                <TableBody>
                    {data.logs.results.map((actionItem) => (
                        <TableRow>
                            ...
                            <TableCell>{actionItem.objectRepr}</TableCell>
                        </TableRow>
                    ))}
                </TableBody>
            </Table>
