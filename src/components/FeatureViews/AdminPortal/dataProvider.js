...
const MUTATION_ADD = {
    ...
    dateRangeDiscounts: gql`
        mutation CreateDateRangeDiscount(
            $active: Boolean
            $description: String
            $name: String
            $amount: Float
    ...
    `,
    paymentMethodDiscounts: gql`
        mutation CreatePaymentMethodDiscount(
            $active: Boolean
            $description: String
            $name: String
            $amount: Float
...
const MUTATION_UPDATE = {
    ...
    `,
    dateRangeDiscounts: gql`
        mutation UpdateDateRangeDiscount(
            $active: Boolean
            $description: String
            $name: String
            $amount: Float
    ...
    `,
    paymentMethodDiscounts: gql`
        mutation UpdatePaymentMethodDiscount(
            $active: Boolean
            $description: String
            $name: String
            $amount: Float
...
export default {
    getList: async (
        resource,
        { pagination: { page, perPage }, sort: { field, order } }
    ) => {
        try {
            ...
            const fieldTypes = Object.fromEntries(
                typeResponse.data.__type.fields.map(({ name, type }) => [
                    name,
                    type.ofType?.name,
                ])
            );
            ...
            const sortedData = [...records].sort((cat1, cat2) => {
                switch (fieldTypes[field]) {
                    ...
                    case 'String': {
                        const comparison = cat1[field].localeCompare(
                            cat2[field]
                        );
