...
const Scheduler = (props) => {
  ...
  const prevState = JSON.parse(sessionStorage.getItem("schedulerState")) || {};
  ...
  const calendarEvents = useMemo(() => {
    ...
    if (
      ...
      courseSessionsArray
        .filter(({ course }) => courses[course].course_type !== courseType)
        .forEach((session) => {
  ...
  return (
    <Grid item xs={12} container>
      ...
      >
        <Grid item xs={4}>
          <Grid className="scheduler-header-firstSet" container direction="row">
            <Grid item>
              <IconButton onClick={() => changeView("timeGridDay")}>
                <CalendarIcon
                  style={{
                    color:
                      view.toLowerCase().includes("grid") && secondaryFontColor,
                  }}
                />
              </IconButton>
            </Grid>
