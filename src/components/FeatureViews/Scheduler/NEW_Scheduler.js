...
const CustomToolbar = (toolbar) => {
	const {schedulerState: {timeFrame}, updateSchedulerState} = useContext(SchedulerContext);

	const setView = (timeFrame, pastDate, delta) => ({
		'day': new Date(pastDate.getFullYear(), pastDate.getMonth(), pastDate.getDate() + delta),
		'week': new Date(pastDate.getFullYear(), pastDate.getMonth(), pastDate.getDate() + delta * 7),
		'month': new Date(pastDate.getFullYear(), pastDate.getMonth() + delta, pastDate.getDate()),
	}[timeFrame]);
...
export default function NEWScheduler() {
	...
	const {data, loading, error} = useQuery(GET_SESSIONS, {
		...
		onCompleted: (data) => {
			const {sessions} = data;
			const parsedBigCalendarSessions = sessions.map(({id, endDatetime, startDatetime, title}) => ({
				id,
				title,
	...
	return (<SchedulerContext.Provider value={{schedulerState, updateSchedulerState}}>
