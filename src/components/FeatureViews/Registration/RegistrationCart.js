...
const RegistrationCart = () => {
    ...
    const courseIDs = useMemo(() => Object.values(registered_courses).flat()
    ...
    useEffect(() => {
        if (registered_courses && CurrentParent) {
            ...
            sessionStorage.setItem("CurrentParent",
            ...
            if (!(course && course.course_type === "class")) {
                ...
                if (course) {
                    finalVal += weeklySessionsParser(
                        course.schedule.start_date, course.schedule.end_date
                    );
                }
    ...
    const studentRegistrations = Object.keys(registered_courses)
        .map((student_id) => studentAccounts[student_id] &&
                ...
                <Grid container item xs={12}>
                    {registered_courses[student_id]
                        ...
                        .map(({new_course, course_id}) => {
                            ...
                            const {checked, sessions, validated} =
                                selectedCourses[student_id][course_id];
                            const start = new Date(course.schedule.start_date)
                                .toLocaleDateString("en-US", dateOptions);
                            const end = new Date(course.schedule.end_date)
                                .toLocaleDateString("en-US", dateOptions);

                            return (
                                <Grid item key={course.course_id} xs={12}>
                                    <Grid alignItems="center" component={Paper}
                                        container square>
                                        <Grid item xs={1}>
    ...
    const selectedCourseOptions = () => {
        ...
        if (selectedCourseID !== -1) {
            return (
                <Payment isOneCourse={displaySelectionOptions === 1}
