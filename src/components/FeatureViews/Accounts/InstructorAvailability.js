...
const InstructorAvailability = ({instructorID, button = true}) => {
		...
		try {
			...
			const postResponses = await Promise.all(
				edited
					.filter(({id}) => !id)
					.map((avail) =>
						instance.post(endpoint, convertAvailObj(avail, instructorID))
					)
			);
			...
			dispatch({
				payload: {
					response: {
						data: patchResponses.concat(postResponses).map(({data}) => data),
					},
				},
	...
	return (
		<>
			...
			>
				<DialogContent>
					<div className="title">Schedule Instructor Availability</div>
					<div className="instructor">Instructor: {instructor.name}</div>
