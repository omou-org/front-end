...
const TutoringInstructorSelect = (props) => {
    ...
    return (
        <>
            {data.map((instructorAvailability) => (
                <label
                    key={instructorAvailability.instructor.id}
                    className={`instructor-select-label 
							   ${
                                   values.instructor ===
                                   instructorAvailability.instructor.id.toString()
                                       ? 'selected'
                                       : ''
                               }`}
                >
                    ...
                    <div
                        style={{ fontSize: '12px' }}
                    >{`${instructorAvailability.instructor.biography}`}</div>
                </label>
            ))}
        </>
    );
...
const TutoringTimeSelect = React.memo((props) => {
    ...
    return (
        <div
            style={{
                border: '#EBFAFF 3px solid',
                borderRadius: '5px',
                padding: '16px',
            }}
        >
            {instructorAvailabilitiesEntries.map(
                ([dayOfWeek, times], index) => (
                    <DayOfWeekTimeSelect
                        key={index}
                        dayOfWeek={dayOfWeek}
                        times={times}
...
const TutoringInstructorAndDateTimeSelector = () => {
    ...
    if (!values.duration)
        return (
            <Grid item>
                <Typography>
                    No information available, please select day and duration of
                    meeting before moving forward.
                </Typography>
            </Grid>
        );

    const selectedInstructorAvailabilities = (instructorId) =>
        data.find(
            (InstructorAndAvailability) =>
                InstructorAndAvailability.instructor.id == instructorId
        )?.tutoringAvailability;
