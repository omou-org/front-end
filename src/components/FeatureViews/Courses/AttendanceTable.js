            ...
            const populatedCourseAttendanceMatrix = enrollments
                .map((enrollment) =>
                    createStudentAttendanceRow(
                        ...
                        studentAttendanceRowsData[
                            enrollment.student.user.id
                        ].sort(sortBySessionId)
                    )
                )
                        ...
                        {sessions
                            ...
                            .map(({ startDatetime, id: sessionId }, index) => (
                                <TableCell
                                    ...
                                    key={sessionId}
                                >
                                    {`Session ${index + 1} (${moment(
                                        startDatetime
                                    ).format('MM/DD/YYYY')})`}
                                    {checkAttendanceEditStateToRenderCheckbox(
                                        sessionId
                                    ) ? (
                                        <Checkbox
                                            checked={
                                                isCheckBoxEditing[sessionId]
                                            }
                                            onChange={handleEdit}
                                            checkedIcon={
                                                <Check
