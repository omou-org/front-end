...
const addField = (prevState, path) => {
    ...
    const SectionFieldList = getSectionFieldList(
        path,
        prevState.registration_form
    );
    const fieldName = SectionFieldList[fieldIndex].field;
    const numFieldType = SectionFieldList.reduce(
        (total, { field }) => total + (field === fieldName),
        1
    );
    const NewField = {
...
function removeField(prevState, path, fieldIndex, conditional) {
    const NewState = prevState;
    let SectionFieldList = getSectionFieldList(
    ...
    SectionFieldList = SectionFieldList.slice(0, fieldIndex).concat(
        SectionFieldList.slice(fieldIndex + 1)
    );
    ...
    const isEnrolled = existingEnrollmentsByStudents
        .map((studentEnrollments) =>
            studentEnrollments.filter((enrollment) =>
                arraysMatch(enrollment, [studentId, courseId])
            )
        )
...
const addTutoringRegistration = (prevState, form) => {
    ...
    const courseName = `${instructorName.substring(
        0,
        instructorName.indexOf(' ')
    )} x ${studentName.substring(0, studentName.indexOf(' '))} - ${subject}`;
    ...
    const isStudentCurrentlyRegistered = prevState.registered_courses
        ? Object.keys(prevState.registered_courses).includes(
              studentID.toString()
          )
        : false;
    ...
    const enrollmentObject = {
        ...
        course_id: `T${
            isStudentCurrentlyRegistered
                ? (
                      prevState.registered_courses[studentID].length + 1
                  ).toString()
                : '0'
        }`,
...
const addStudentRegistration = (
    ...
    const isStudentCurrentlyRegistered = registeredCourses
        ? Object.keys(registeredCourses).includes(studentID.toString())
        : false;
...
const editCourseRegistration = (prevState, course) => {
    ...
    let editedRegistration = prevState.registered_courses[student_id].find(
            ...
            case 'course_id':
                if (value !== course_id) {
                    editedRegistration = {
                        ...
                        display: {
                            ...editedRegistration.display,
                            course_name: renderCourseName(
                                editedRegistration.type,
                                form,
                                new_course
                            ),
                        },
    ...
    const updated_registered_courses = prevState.registered_courses[
        student_id
    ].map((registration) => {
    ...
    sessionStorage.setItem(
        'registered_courses',
        JSON.stringify(updatedRegistration.registered_courses)
    );
