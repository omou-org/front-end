...
export const formatCourse = (formCourse, type) => {
  ...
  endDate = new Date(
      endDate.setDate(startDate.getDate() + 7 * (numberOfSessions - 1))
  );
  ...
  endTime = new Date(
      endTime.setTime(
          endTime.getTime() + durationParser[tuitionInfo.Duration] * 60 * 60 * 1000
      )
  );
  ...
  return {
    ...
    is_confirmed:
        courseInfo["Did instructor confirm?"] === "Yes, Instructor Confirm",
