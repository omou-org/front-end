	...
	useEffect(() => {
		...
		(async () => {
			if (
				...
				const existingTutorings = tutoringRegistrations.filter(({courseID}) =>
					isExistingTutoring(courseID)
				);
				...
				try {
					...
					const enrollmentSessions = (index) => {
						...
						return tutoringRegistrations[index - classRegistrations.length]
							.sessions;
					};
			...
			(async () => {
				try {
					...
					const studentIDs = Payment.data.registrations.map(
						(registration) => registration.enrollment_details.student
					);
